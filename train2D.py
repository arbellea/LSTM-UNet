...
def train():
    ...
    seg_measure = losses.SegMeasure(params.channel_axis + 1, temporal=True, three_d=False)
    ...
    if True:
        ...
        for _ in range(int(ckpt.step), params.num_iterations + 1):
            image_sequence, seg_sequence, _, is_last_batch, fnames_batch = train_data_provider.get_batch()
            if params.profile:
            ...
            if not int(ckpt.step) % params.validation_interval:
                ...
                val_image_sequence, val_seg_sequence, _, val_is_last_batch, val_fnames_batch = val_data_provider.get_batch()
                val_output_sequence, val_predictions, val_loss_value = val_step(val_image_sequence, val_seg_sequence)
                model.reset_states_per_batch(val_is_last_batch)  # reset states for sequences that ended
                if not params.dry_run:
                    seg_onehot = tf.one_hot(tf.cast(tf.squeeze(val_seg_sequence[:, -1], params.channel_axis), tf.int32),
                                            depth=3)
                    if params.channel_axis == 1:
                        seg_onehot = tf.transpose(seg_onehot, (0, 3, 1, 2))
